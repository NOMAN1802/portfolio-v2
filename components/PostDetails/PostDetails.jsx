...
const PostDetails = ({post}) => {
  ...
  return (
    <>
      ...
      <AnimatedText text={`Learn more about ${post?.title}`} textStyles="h2 font-primary my-8" />
  ...
  {post?.resourceLink && (
    <TooltipProvider>
      <Tooltip>
        <TooltipTrigger asChild>
          <Link href={post?.resourceLink} target="_blank" className="inline-flex items-center space-x-2 px-4 py-2 bg-gradient-to-r from-purple-300 to-gray-400 text-white rounded-lg shadow-lg transform transition-all duration-300 hover:scale-105 hover:shadow-2xl">
    ...
    <TooltipProvider>
      <Tooltip>
        <TooltipTrigger asChild>
          <Link href="/"  className="inline-flex items-center space-x-2 px-4 py-2 bg-gradient-to-r from-green-500 to-teal-500 text-white rounded-lg shadow-lg transform transition-all duration-300 hover:scale-105 hover:shadow-2xl">
            <FaLink className="text-xl" />
        ...
        <TooltipContent>
          ...
          <div className="w-full lg:w-1/2 lg:pl-8 space-y-6">
            <h1 className="text-3xl lg:text-4xl font-bold mb-2 text-gray-800 font-primary">{post?.title}</h1>
            <h2 className="text-xl lg:text-2xl font-semibold mb-4 text-gray-600">{post?.category}</h2>
            <p className="text-lg text-gray-700 leading-relaxed" dangerouslySetInnerHTML={{ __html: sanitizedPostDetails }}></p>
